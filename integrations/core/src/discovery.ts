...
export interface DiscoveryIndex {
  ...
  features: Array<{ name: string; description: string; filePaths: string[] }>;
...
export function loadDiscoveryFromWorkspace(workspacePath: string, repoPath: string): DiscoveryIndex | null {
  ...
  try {
    const raw = JSON.parse(readFileSync(workspacePath, "utf8")) as {
 ...
 */
export function deriveEntryChanges(
  repoRoot: string,
  entries: DiscoveryEntry[],
): { changed: DiscoveryEntry[]; missing: DiscoveryEntry[] } {
  const changed: DiscoveryEntry[] = [];
  const missing: DiscoveryEntry[] = [];
  for (const entry of entries) {
    ...
    try {
      ...
      if (currentHash !== entry.contentHash) changed.push(entry);
    } catch {
...
export function buildDiscoveryContextPacket(
  discovery: DiscoveryIndex,
  options: { repoRoot: string; taskHint?: string },
): string {
  ...
  if (taskHint !== undefined && taskHint.trim() !== "") {
    ...
    if (relevant.length > 0) {
      ...
      for (const entry of relevant) parts.push(`  ${entry.filePath} — ${entry.purpose}`);
    }
  }
  ...
  const scored = entries
    .map((entry) => {
      const entryTokens = tokenize(
        [entry.filePath, entry.purpose, ...entry.features, ...entry.importantExports].join(" "),
      );
      ...
      const score = hintTokens.filter((hintToken) =>
        entryTokens.some((entryToken) => entryToken.startsWith(hintToken) || hintToken.startsWith(entryToken)),
      ).length;
      return { entry, score };
    })
...
export function buildDiscoveryLine(repoRoot: string, discovery: DiscoveryIndex | null): string {
  if (!discovery) return "No discovery data. Run Discover Project to enable incremental indexing.";

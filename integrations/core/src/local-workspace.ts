...
interface LocalActivity {
  id: string;
  actor: string;
  actorKind: "claude" | "codex" | "agent" | "human";
  action: string;
  target: string;
  time: string;
...
interface LocalBoard {
  ...
  currentPhase?: string;
  decisions?: Array<{ id: string; decision: string; reason: string; status: "Accepted" | "Proposed"; title: string }>;
  lastActivity?: string;
  phases?: Array<{ id: string; name: string; objective: string; progress: number; status: "complete" | "in_progress" | "planned" }>;
  progress?: number;
}
...
export function defaultLocalWorkspacePath(platform = process.platform, home = homedir()): string {
  if (platform === "darwin") return join(home, "Library", "Application Support", "Relay", "relay-data", "workspace.json");
  if (platform === "win32") {
...
export function appendCheckpointToLocalWorkspace(
  workspacePath: string,
  checkpoint: CheckpointInput,
): LocalWorkspaceCheckpointResult {
  if (!existsSync(workspacePath)) return { updated: false, reason: "workspace_missing" };

  const workspace = JSON.parse(readFileSync(workspacePath, "utf8")) as Partial<LocalWorkspace>;
  ...
  if (board === undefined) return { updated: false, reason: "board_not_found" };
  ...
  if (content !== "" && !context.some((item) => item.id === contextId)) {
    const confidence: LocalContext["confidence"] = checkpoint.kind === "decision_made" || checkpoint.kind === "blocker_discovered" ? "High" : "Medium";
    board.context = [
      {
        id: contextId,
  ...
  writeWorkspace(workspacePath, workspace as LocalWorkspace);
  return { updated: true };
}

export function checkpointFromSessionEndInput(input: unknown, agent: AgentKind, boardId: string): CheckpointInput {
...
function updateCardFromCheckpoint(card: LocalCard, checkpoint: CheckpointInput): void {
...
function updatePhaseTimeline(board: LocalBoard, checkpoint: CheckpointInput, card?: LocalCard): void {
...
function updateBoardMetrics(board: LocalBoard): void {
...
function hasDecision(board: LocalBoard, idempotencyKey: string): boolean {
  return (board.decisions ?? []).some((item) => item.id === `decision-${idempotencyKey}`);
}

function averageProgress(cards: LocalCard[]): number {
...
function writeWorkspace(path: string, workspace: LocalWorkspace): void {
...
function agentActorKind(agent: AgentKind): LocalActivity["actorKind"] {
  if (agent === "claude-code") return "claude";
  if (agent === "codex") return "codex";
  return "agent";
}

function activityAction(kind: CheckpointInput["kind"]): string {
  if (kind === "decision_made") return "recorded decision";
  if (kind === "blocker_discovered" || kind === "tests_failed") return "flagged";
  if (kind === "tests_passed" || kind === "card_completed") return "verified";
  if (kind === "files_changed") return "changed files for";
  return "checkpointed";
}

function activityTone(kind: CheckpointInput["kind"]): LocalActivity["tone"] {
  if (kind === "blocker_discovered" || kind === "tests_failed") return "orange";
  if (kind === "decision_made" || kind === "tests_passed" || kind === "card_completed") return "green";
  if (kind === "handoff" || kind === "session_end") return "violet";
  return "blue";
}

function contextCategory(kind: CheckpointInput["kind"]): LocalContext["category"] {
  if (kind === "decision_made") return "Decision";
  if (kind === "blocker_discovered" || kind === "tests_failed") return "Warning";
  if (kind === "files_changed") return "Important file";
  return "Current state";
}

function contextTitle(checkpoint: CheckpointInput): string {
  if (checkpoint.kind === "files_changed" && checkpoint.filesChanged[0] !== undefined) return `Changed ${checkpoint.filesChanged[0]}`;
  return checkpoint.summary.slice(0, 90);
}

function checkpointContent(checkpoint: CheckpointInput): string {
  const parts = [
    checkpoint.summary,
